...
const Versus = () => {
  ...
  const getSafeUnitData = (id) => {
    ...
    if (id === 'leader2') return { id: 'leader2', name: "Queen (P2)", unitImg: roiPlayer2Coin || roiPlayer1Coin, cardImg: reineCard || roiCard, type: "Essential", desc: "Move 1 space." };
    try {
  ...
  const executeAction = (targetR, targetC, action) => {
    ...
    if (type === 'move') {
      ...
      newBoard[targetR][targetC] = null;
        ...
        const tNeighbors = getNeighbors(targetR, targetC).filter(n => !newBoard[n[0]][n[1]]);
    ...
    if (winners) { setGameOver(winners); setGameLog(`${winners} Wins!`); }
  };
  ...
  const finalizeRecruit = (r, c, cardId, owner) => {
    const newBoard = board.map(row => row.map(cl => cl ? {...cl, isNew: false} : null));
  ...
  const endTurnLogic = (phase, currentBoard = board) => {
    const count = (o) => currentBoard.flat().filter(u => u && u.owner === o && u.cardId !== 'cub').length;
  ...
  const resetForP1 = (b) => {
    const resetBoard = b.map(row => row.map(c => c ? {...c, moved: false, isNew: false} : null));
    setBoard(resetBoard);
    setTurn(1);
    setGameLog("Player 1 Turn");
  };
    ...
    <div className="w-full h-[100dvh] overflow-hidden flex flex-col font-serif select-none relative bg-cover bg-center" style={{ backgroundImage: `url(${gameBackground})` }}>
      ...
      <div className="h-14 md:h-16 bg-[#2a1e1a] border-b-4 border-[#8D6E63] shadow-2xl flex items-center justify-between px-4 md:px-8 z-50 shrink-0 relative">
        ...
        <div className="flex gap-3">
          <button onClick={() => endTurnLogic(turn, board)} className={`px-4 py-2 rounded-lg shadow border text-sm font-bold uppercase text-white transition-colors ${(turn === 1 || turn === 3) ? "bg-amber-600 hover:bg-amber-500 border-amber-400" : "bg-gray-600 hover:bg-gray-500 border-gray-400"}`}>
             {(turn === 1 || turn === 3) ? "End Action" : "Skip Recruit"}
          </button>
          <button onClick={() => navigate("/")} className="bg-gradient-to-b from-[#5D4037] to-[#3E2723] hover:from-[#6D4C41] text-[#EFEBE9] border border-[#8D6E63] px-4 py-2 rounded-lg font-semibold text-sm shadow-md">Exit</button>
        </div>
      </div>
      ...
      <div className="flex-grow flex flex-col md:flex-row w-full h-full overflow-hidden relative">
        ...
        <div className={`absolute md:static inset-0 bg-[#1a1210]/95 backdrop-blur-sm z-40 flex flex-col shadow border-r-4 border-[#5D4037] transition-transform duration-300 md:transform-none md:w-72 lg:w-80 shrink-0 ${mobileTab === "recruit" ? "translate-y-0" : "translate-y-full md:translate-y-0 hidden md:flex"}`}>
          <div className="p-4 bg-[#2a1e1a] border-b border-[#5D4037] flex justify-between items-center shadow-md shrink-0">
            <h2 className="text-[#FFCA28] font-bold text-sm md:text-base uppercase tracking-[0.2em]">Market</h2>
          ...
          <div className="flex-grow overflow-y-auto no-scrollbar p-4 md:p-6 bg-opacity-10">
            <div className="flex flex-wrap md:flex-col justify-center items-center gap-4 md:gap-6 min-h-min">
              {visibleDeck.map((card, idx) => (
                <CardUI key={idx} data={card} onClick={() => { if (turn === 2 || turn === 4) { setRecruitSelectionIndex(idx); setMobileTab(null); setGameLog("Place unit on valid zone"); } }} isSelected={(turn === 2 || turn === 4) && recruitSelectionIndex === idx} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="flex-grow relative flex items-center justify-center p-2 md:p-6 lg:p-10 overflow-hidden">
          ...
          <div className="relative w-full h-full flex items-center justify-center z-10">
            <div className="relative max-h-full max-w-full aspect-[650/750] transition-all duration-500">
              <img src={board_img} alt="Board" className="w-full h-full object-contain pointer-events-none drop-shadow-2xl z-0 relative" />

              {board.map((row, r) =>
                row.map((cell, c) => {
                  if (!SLOT_COORDINATES[r] || !SLOT_COORDINATES[r][c]) return null;
                  const coords = SLOT_COORDINATES[r][c];
                  const actionMove = validMoves.find(m => m.r === r && m.c === c);
                  const isRecruitValid = (turn === 2 && isRecruitZone(r, c, 1) && !cell) || (turn === 4 && isRecruitZone(r, c, 2) && !cell);
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                          if (actionMove) executeAction(r, c, actionMove);
                          else if (isRecruitValid && recruitSelectionIndex !== null) {
                              ...
                              if (card.id === 'hermit' && recruitStep === 0) {
                                  ...
                                  setBoard(nb); setRecruitStep(1); setGameLog("Place the Cub");
                              } else {
                      ...
                      className={`w-[11.5%] aspect-square flex items-center justify-center rounded-full transition-all duration-200 z-20 ${(actionMove || (isRecruitValid && recruitSelectionIndex !== null)) ? "cursor-pointer scale-110" : ""}`}
                    >
                      {actionMove && <div className={`absolute w-full h-full rounded-full border-2 ${actionMove.type.includes('ability') ? 'bg-red-500/30 action-ability' : 'bg-green-500/30 action-move'}`}></div>}
                      {isRecruitValid && recruitSelectionIndex !== null && <div className="absolute w-full h-full rounded-full bg-amber-400/40 animate-pulse border-2 border-amber-400"></div>}
                      ...
                      {cell && unitInfo && (
                        <div className={`relative w-[95%] h-[95%] transition-all duration-300 ease-out ${cell.isNew ? "animate-spawn" : ""} ${cell.moved ? "grayscale-[0.8] opacity-80" : "hover:scale-110 cursor-pointer"}`}>
                          <img src={unitInfo.unitImg} alt="Unit" onError={(e) => { e.target.onerror = null; e.target.src = roiPlayer1Coin; }} className={`w-full h-full object-contain drop-shadow-md ${cell.owner === 2 ? "filter brightness-90 hue-rotate-[140deg]" : ""}`} />
                          <div className={`absolute inset-0 rounded-full border-[3px] shadow-sm ${cell.owner === 1 ? "border-blue-500 shadow-blue-500/50" : "border-red-600 shadow-red-600/50"}`}></div>
                        </div>
                      )}
                    </div>
                  );
        ...
        <div className={`absolute md:static inset-0 bg-[#1a1210]/95 backdrop-blur-sm z-40 flex flex-col shadow border-l-4 border-[#5D4037] transition-transform duration-300 md:transform-none md:w-72 lg:w-80 shrink-0 ${mobileTab === "roster" ? "translate-y-0" : "translate-y-full md:translate-y-0 hidden md:flex"}`}>
          <div className="p-4 bg-[#2a1e1a] border-b border-[#5D4037] flex justify-between items-center shadow-md shrink-0">
            <h2 className={`font-bold text-sm md:text-base uppercase tracking-[0.2em] drop-shadow-sm ${(turn === 1 || turn === 2) ? "text-blue-400" : "text-red-400"}`}>{(turn === 1 || turn === 2) ? "Player 1 Party" : "Player 2 Party"}</h2>
            <button onClick={() => setMobileTab(null)} className="md:hidden text-2xl font-bold text-[#D7CCC8]">&times;</button>
          </div>
          <div className="flex-grow overflow-y-auto no-scrollbar p-4 md:p-6 bg-opacity-10">
            <div className="flex flex-wrap md:flex-col justify-center items-center gap-4 md:gap-6 min-h-min pb-20 md:pb-0">
              {Array(5).fill(null).map((_, idx) => {
                  const filteredRoster = currentRoster.filter(u => u.cardId !== "cub");
         ...
         <div className="md:hidden h-16 bg-[#2a1e1a] flex items-center justify-around shrink-0 z-50 text-[#D7CCC8] border-t-4 border-[#5D4037]">
          <button onClick={() => setMobileTab(mobileTab === "recruit" ? null : "recruit")} className={`flex flex-col items-center p-2 rounded w-20 transition-all ${mobileTab === "recruit" ? "bg-[#5D4037] text-[#FFCA28]" : "opacity-70"}`}>
            <span className="text-xl mb-1">🎴</span><span className="text-[9px] font-bold uppercase">Market</span>
          </button>
          <button onClick={() => setMobileTab(null)} className={`flex flex-col items-center p-2 rounded w-20 transition-all ${mobileTab === null ? "bg-[#5D4037] text-[#FFCA28]" : "opacity-70"}`}>
            <span className="text-xl mb-1">♟️</span><span className="text-[9px] font-bold uppercase">Board</span>
          </button>
          <button onClick={() => setMobileTab(mobileTab === "roster" ? null : "roster")} className={`flex flex-col items-center p-2 rounded w-20 transition-all ${mobileTab === "roster" ? "bg-[#5D4037] text-[#FFCA28]" : "opacity-70"}`}>
            <span className="text-xl mb-1">🛡️</span><span className="text-[9px] font-bold uppercase">Party</span>
          </button>
        </div>
      </div>
    </div>
